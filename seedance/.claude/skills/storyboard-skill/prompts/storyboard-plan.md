...
3. 建立镜头（精简）：
   - 开头：1个场景建立镜头即可
   - 中间：仅情节需要时加入环境镜头
   - ⚠️ 建立镜头人物入镜规则：即使是建立镜头/空镜/环境镜头，也必须根据 source_text 和上下文剧情判断当前场景中是否已有角色在场。如果有角色在场（即该场景内角色已到达且未离场），则 characters 数组中必须包含该角色（远景中可以是渺小身影/剪影），不得设为空数组。只有完全无人的纯环境空镜（如日出、雨景、空荡建筑外观等确认无人的画面）才允许 characters 为空数组。
