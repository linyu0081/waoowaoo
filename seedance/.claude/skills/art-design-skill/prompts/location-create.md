...
7. 【位置标识规则（重要）】
   - 场景图上必须在每个可站位置处标注大写英文字母标识（A、B、C、D...）
   - 在 prompt 的末尾追加位置标注指令，格式：「图上标注位置：A位-{位置描述简称}，B位-{位置描述简称}，...，用醒目的白色大写字母标注在对应位置上」
   - 标识字母必须与 available_slots 数组顺序一一对应（第1个位置=A，第2个=B，以此类推）
   - 这些标识将在后续分镜中用于精确指定角色站位

以下是用户的生成指令:{user_input}

只返回以下json格式，禁止返回一切除json以外的多余内容。⚠️ 所有引号（""''等）在 JSON 字符串值中必须替换为「」，严禁出现未转义的英文双引号 "。
{
  "prompt":"「场景名称」场景描述内容。图上标注位置：A位-xxx，B位-xxx，用醒目的白色大写字母标注在对应位置上",
  "available_slots":[
    {"label":"A", "description":"饭桌左侧靠桌边的位置"},
    {"label":"B", "description":"门口内侧靠墙的位置"}
  ]
}
