...
子形象（同一角色不同状态）使用垫图模式：
先上传基础形象获取 CDN URL，再通过 --input-images 传入参考图生成，
确保同一角色不同状态保持面部一致性。
"""
...
# ===================== CHARACTER PROMPTS =====================
# 基础形象（id=0）：纯文生图，包含完整外貌描述
# 子形象（id>=1）：垫图生成，传入基础形象作为参考图 + 状态变化描述

# --- 基础形象 (无 ref_base) ---
BASE_CHARACTER_TASKS = [
    {
        "name": "char-叶烬-base",
...
# --- 子形象 (有 ref_base，使用垫图模式生成) ---
# ref_base: 对应基础形象的 name，用于查找已生成的基础图片作为参考
# prompt: 以参考图为锚点，描述该角色的具体状态变化
SUB_CHARACTER_TASKS = [
    {
        "name": "char-叶烬-半武装",
        "ref_base": "char-叶烬-base",
        "prompt": "参考图中的角色是叶烬的初始形象。请保持该角色的面部特征和整体身形不变，生成他的Lv.1半武装状态：暗银色铠甲从心口向外延展，覆盖躯干和四肢，左半身刻满精密几何纹路，右半身爬满有机质裂纹，两种纹路在胸口交界处闪烁暗金色光芒。三对暗灰色羽翼从背后展开，翼缘燃烧着暗焰。头顶一圈碎裂的光环发出不稳定的暗金色脉冲。左手黑手套破裂露出黑色硬质甲壳覆盖的手指，指尖是暗金利爪。脚踏暗银色战靴，靴面刻有频率纹路。" + CHAR_COMPOSITION + ART_STYLE_SUFFIX,
    },
    {
        "name": "char-叶烬-全武装",
        "ref_base": "char-叶烬-base",
        "prompt": "参考图中的角色是叶烬的初始形象。请保持该角色的面部特征和整体身形不变，生成他的Lv.2全频同步完全武装状态：全身覆盖暗银与暗金交织的铠甲，左半身几何纹路泛着天使系白光，右半身有机裂纹流动恶魔系暗焰，胸口交界处暗金光芒暴涨。六对暗灰色羽翼完全展开，翼展宽阔，每片羽翼边缘燃烧着黑色暗焰。碎裂光环脉冲加倍形成持续的暗金恒光。左手完全暴露，黑色硬质甲壳延伸至前臂，指尖暗金利爪，关节缝隙闪烁双频能量。脚踏全甲战靴，靴面纹路发出暗金光芒。" + CHAR_COMPOSITION + ART_STYLE_SUFFIX,
    },
    {
        "name": "char-灵曦-半降临",
        "ref_base": "char-灵曦-base",
        "prompt": "参考图中的角色是灵曦的初始形象。请保持该角色的面部特征和整体身形不变，生成她的半降临状态：白金色光甲从右肩开始生长覆盖右半身，几何结构精密到每个菱形切面都严丝合缝，光甲沿锁骨蔓延至右臂、右胸、右腰。三对光翼从背后绽开——仅展开一对半，共三翼，每片光翼发出灼目白光。眉心金色六芒纹更加清晰。仪式白袍左肩处有撕裂破损。脚穿白色软底长靴，靴面沾有尘土。" + CHAR_COMPOSITION + ART_STYLE_SUFFIX,
    },
    {
        "name": "char-灵曦-完全降临",
        "ref_base": "char-灵曦-base",
        "prompt": "参考图中的角色是灵曦的初始形象。请保持该角色的面部特征和整体身形不变，生成她的完全降临瞬间状态：三翼暴涨为六翼，纯白光翼从背后炸裂展开，几何结构精密到令人窒息，每片翼面都在发出灼目圣光。全身被白金光甲完整覆盖，甲胄造型神圣庄严。完整的炽天使光环从头顶涌出，直径五米，表面火焰文字疯狂流转。白金全甲战靴包裹双足，靴面圣纹发光。" + CHAR_COMPOSITION + ART_STYLE_SUFFIX,
    },
]
...
def process_task_batch(tasks, results, task_type="char", input_images_map=None):
    """Submit, poll, and download a batch of tasks.
    input_images_map: dict of {task_name: ref_image_url} for reference-based generation.
    Returns dict of {task_name: local_file_path} for successfully generated images.
    """
    input_images_map = input_images_map or {}
    downloaded = {}

    # Submit all tasks
    pending = []
    for task in tasks:
        ref_url = input_images_map.get(task["name"])
        task_id = generate_image(task["prompt"], task["name"], "16:9", input_images=ref_url)
        if task_id:
            pending.append({"name": task["name"], "task_id": task_id})
        else:
...
def main():
    ...
    # ===== Phase 1: 基础形象 + 场景 (纯文生图) =====
    print("=" * 60)
    print("Phase 1: 生成基础形象和场景图（纯文生图）")
    print("=" * 60)

    print("\n--- 基础角色形象 ---")
    base_downloaded = process_task_batch(BASE_CHARACTER_TASKS, results, "char")

    print("\n--- 场景图 ---")
    process_task_batch(SCENE_TASKS, results, "scene")

    # ===== Phase 2: 子形象 (垫图生成) =====
    if SUB_CHARACTER_TASKS:
        print("\n" + "=" * 60)
        print("Phase 2: 生成子形象（垫图模式，参考基础形象）")
        print("=" * 60)

        # 为每个子形象上传对应的基础图片获取 CDN URL
        ref_urls = {}  # {sub_task_name: cdn_url}
        upload_cache = {}  # {base_name: cdn_url} 避免重复上传

        for task in SUB_CHARACTER_TASKS:
            ref_base = task["ref_base"]
            base_path = base_downloaded.get(ref_base) or os.path.join(IMAGES_DIR, f"{ref_base}.png")

            if not os.path.exists(base_path):
                print(f"  WARNING: 基础形象 {ref_base} 不存在 ({base_path})，子形象 {task['name']} 将退回纯文生图")
                continue

            if ref_base not in upload_cache:
                cdn_url = upload_image(base_path)
                if cdn_url:
                    upload_cache[ref_base] = cdn_url
                else:
                    print(f"  WARNING: 上传 {ref_base} 失败，子形象 {task['name']} 将退回纯文生图")
                    continue

            ref_urls[task["name"]] = upload_cache[ref_base]

        print(f"\n  已上传 {len(upload_cache)} 张基础形象，准备生成 {len(SUB_CHARACTER_TASKS)} 个子形象")

        process_task_batch(SUB_CHARACTER_TASKS, results, "char", input_images_map=ref_urls)

    # ===== Phase 3: 重试失败的提交 =====
    failed_submits = [f for f in results["failed"] if f["reason"] == "submit_failed"]
    if failed_submits:
        print("\n" + "=" * 60)
        print("Phase 3: 重试失败的提交")
        print("=" * 60)
        all_task_map = {t["name"]: t for t in BASE_CHARACTER_TASKS + SUB_CHARACTER_TASKS + SCENE_TASKS}
    ...
    # ===== Summary =====
    total = len(BASE_CHARACTER_TASKS) + len(SUB_CHARACTER_TASKS) + len(SCENE_TASKS)
    print("\n" + "=" * 60)
    print("FINAL RESULTS")
    print("=" * 60)
    print(f"\nSuccess: {len(results['success'])}/{total}")
